...
=#
            E_adv_hat, F_adv_hat, G_adv_hat =
                AUSM(Qbase,Qcon,cellxmax,cellymax,cellzmax,vecAx,vecAy,vecAz,specific_heat_ratio,volume)
            ...
            #=
            println(mu[2,2,2])
            println(lambda[2,2,2])

            println(Qbase[2,2,2,:])
            println(Qcon[2,2,2,:])
            println(Qcon_hat[2,2,2,:])

            println(E_adv_hat[2,2,2,:])
            println(E_vis_hat[2,2,2,:])
        ...
        elseif time_integ == "2"
            ...
            for tau in 1:in_nt
                ...
                #E_adv_hat, F_adv_hat, G_adv_hat =
                 #   AUSM(Qbasem,Qcon,cellxmax,cellymax,cellzmax,vecAx,vecAy,vecAz,specific_heat_ratio,volume)
                #=
                println(" AUSM ")
                println(E_adv_hat[2,2,2,:])
                ...
                while true
                    ...
                    output_innertime(fwrite, tau, norm2)
        ...
        if round(evalnum) % every_outnum == 0
            ...
            output_result(evalnum, Qbase, cellxmax, cellymax, cellzmax, out_file_front, out_ext, out_dir, Rd)
        end
