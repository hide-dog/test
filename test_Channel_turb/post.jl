    ...
    for i in 1:length(inf)
        if occursin(".dat", inf[i]) == true
            ...
            for j in 2:length(fff)
                ...
                yplus[j-1] = parse(Float64,temp[7])
            end
            ...
            write_result(yplus,out_file,outdir,7)
...
function write_cells(elements,out_file,outdir)
    ...
    open(fff,"a") do f
        ...
        for i in 1:a
            ...
            d5 = @sprintf("%1.0f", elements[i,5])
            d6 = @sprintf("%1.0f", elements[i,6])
            d7 = @sprintf("%1.0f", elements[i,7])
            d8 = @sprintf("%1.0f", elements[i,8])
