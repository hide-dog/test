    ...
    for i in 1:length(inf)
        if occursin(".dat", inf[i]) == true
            ...
            for j in 2:length(fff)
                ...
                N2[j-1] = parse(Float64,temp[5])
                N[j-1] = parse(Float64,temp[6])
                T[j-1] = parse(Float64,temp[7])
            end
            ...
            write_result(N2,out_file,outdir,5)
            write_result(N,out_file,outdir,6)
            write_result(T,out_file,outdir,7)
